...
find_major_chunk(ns_gc* gc, ns_byte* ptr) {
  ns_byte* curr;
  for (curr = &gc->major_heap[0]; !(ptr >= curr && (ptr < (curr + CHUNK_SIZE(curr)))) &&
                                  curr < &gc->major_heap[GC_MAJHEAP_SIZE]; curr += CHUNK_SIZE(curr));
...
major_alloc(ns_gc* gc, size_t size) {
  ...
  ns_byte* curr;
  for (curr = &gc->major_heap[0]; (CHUNK_FLAGS(curr) > GC_FREE && size > CHUNK_SIZE(curr)); curr += CHUNK_SIZE(curr));
...
darken_major(ns_gc* gc) {
  ns_byte* curr;
  for (curr = &gc->major_heap[0]; curr != 0 && curr < &gc->major_heap[GC_MAJHEAP_SIZE]; curr += CHUNK_SIZE(curr)) {
      ...
      if (ref != 0) {
        if (POINTS_MINOR(*ref)) {
          ...
          if (MARKED(chunk)) {
            ...
            if (new_ptr != 0) *ref += (new_ptr - chunk);
          }
        }
      }
    }
...
copy_minor_heap(ns_gc* gc) {
  for (int i = 0; i < gc->free_chunk; i++) {
    ...
    if(MARKED(chunk)){
      ...
      for(int j = 0; j < PTR_INDEX(CHUNK_SIZE(chunk)); j++){
        if(REF_PTR_MIN(*BITS_AT(chunk, j))){
          void** p = BITS_AT(chunk, j);
          ns_byte* ref_chunk = MINOR_CHUNK(*p);
          if(MARKED(ref_chunk)){
            ns_gc_add_ref(gc, p);
          }
        }
...
ns_gc_print_minor(ns_gc* gc) {
  ...
  for (int i = 0; i < gc->free_chunk; i++) {
    ns_byte* chunk = CHUNK_AT(i);
    printf("  Chunk: %.4d  Size: %.3d  Marked: %c\n", ((int) CHUNK_OFFSET(chunk)), CHUNK_SIZE(chunk),
           (MARKED(chunk) ? 'y' : 'n'));
  }
